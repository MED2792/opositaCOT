...
// (por ejemplo, en local antes de crear el proyecto, o si se olvidan en
// Vercel), "supabase" queda en null y toda la parte de login se oculta sola
...
// Subtemas seleccionables dentro de un bloque (de momento solo Miembro
// superior). Cada pregunta de ese bloque lleva un campo "subtopic" con uno
// de estos ids.
const SUBTOPICS = {
  ms: [
    { id: "hombro", label: "Hombro y cintura escapular" },
    { id: "codo", label: "Codo" },
    { id: "antebrazo_muneca", label: "Antebrazo y muñeca" },
    { id: "mano", label: "Mano y dedos" },
    { id: "infantil_ms", label: "Miembro superior infantil" },
  ],
  cadera: [
    { id: "fracturas_femur_proximal", label: "Fracturas de fémur proximal y diáfisis" },
    { id: "fracturas_pelvis_acetabulo", label: "Fracturas de pelvis y acetábulo" },
    { id: "artroplastia_cadera", label: "Artroplastia de cadera" },
    { id: "patologia_no_traumatica_cadera", label: "Patología no traumática de cadera" },
    { id: "luxacion_cadera", label: "Luxación traumática de cadera" },
  ],
  raquis: [
    { id: "trauma_raquis", label: "Traumatismo raquimedular" },
    { id: "deformidades_raquis", label: "Deformidades (escoliosis y cifosis)" },
    { id: "cervical_degenerativa", label: "Patología cervical degenerativa" },
    { id: "lumbar_degenerativa", label: "Patología lumbar degenerativa" },
    { id: "anatomia_exploracion_raquis", label: "Anatomía, biomecánica y exploración" },
  ],
  rodilla: [
    { id: "fracturas_rodilla", label: "Fracturas alrededor de la rodilla" },
    { id: "ligamentos_rodilla", label: "Lesiones ligamentosas" },
    { id: "meniscos_rodilla", label: "Lesiones meniscales" },
    { id: "femoropatelar", label: "Patología femoropatelar" },
    { id: "luxacion_rodilla", label: "Luxación de rodilla" },
  ],
  infantil: [
    { id: "ddc", label: "Displasia del desarrollo de la cadera" },
    { id: "coxa_vara_deficiencia_femoral", label: "Coxa vara y deficiencia femoral" },
    { id: "perthes", label: "Enfermedad de Perthes" },
    { id: "cadera_dolorosa_efp", label: "Cadera dolorosa aguda y EFP" },
  ],
};


// Progresión tipo carrera médica, basada en el total de respuestas acertadas
// acumuladas (histórico de todas las sesiones, todos los bloques).
const LEVELS = [
  { min: 0, name: "Estudiante" },
  { min: 50, name: "Residente R1" },
  { min: 100, name: "Residente R2" },
  { min: 200, name: "Residente R3" },
  { min: 350, name: "Residente R4" },
  { min: 550, name: "Residente R5" },
  { min: 800, name: "Adjunto" },
  { min: 1200, name: "Jefe de Sección" },
  { min: 2000, name: "Jefe de Servicio" },
];
...
function getLevelInfo(totalCorrect) {
  ...
  for (let i = 0; i < LEVELS.length; i++) {
    if (totalCorrect >= LEVELS[i].min) {
      current = LEVELS[i];
      next = LEVELS[i + 1] || null;
    }
  }
  const pct = next
    ? Math.min(100, Math.round(((totalCorrect - current.min) / (next.min - current.min)) * 100))
    : 100;
  return { current, next, pct };
}

// Insignias por hitos: se calculan en cada render a partir de datos ya
// guardados (son todos acumulativos, así que una vez conseguida una
// insignia no puede "perderse").
const FIXED_BADGES = [
  {
    id: "first_step",
    icon: "🔰",
    name: "Primeros pasos",
    desc: "Consigue tu primer acierto",
    check: (ctx) => ctx.totalCorrectEver >= 1,
  },
  {
    id: "explorer",
    icon: "🧭",
    name: "Explorador",
    desc: "Responde al menos una pregunta de cada bloque del temario",
    check: (ctx) => ctx.blocksExplored >= BLOCKS.length,
  },
  {
    id: "hundred",
    icon: "💯",
    name: "Centenar",
    desc: "Alcanza 100 aciertos acumulados",
    check: (ctx) => ctx.totalCorrectEver >= 100,
  },
  {
    id: "fivehundred",
    icon: "🥈",
    name: "Quinientos aciertos",
    desc: "Alcanza 500 aciertos acumulados",
    check: (ctx) => ctx.totalCorrectEver >= 500,
  },
  {
    id: "thousand",
    icon: "🥇",
    name: "Mil aciertos",
    desc: "Alcanza 1.000 aciertos acumulados",
    check: (ctx) => ctx.totalCorrectEver >= 1000,
  },
  {
    id: "streak7",
    icon: "🔥",
    name: "Constancia",
    desc: "Llega a una racha de 7 días seguidos",
  ...
  },
  {
    ...
    desc: "Llega a una racha de 30 días seguidos",
  ...
  },
  {
    ...
    desc: "Termina una sesión de al menos 10 preguntas con el 100% de aciertos",
    check: (ctx) => ctx.hadPerfectSession,
  },
];

const BLOCK_BADGES = BLOCKS.map((b) => ({
  id: `master_${b.id}`,
  icon: "🏅",
  name: `Especialista en ${b.label}`,
  desc: `Consigue al menos un 85% de acierto histórico con 20 o más preguntas respondidas en ${b.label}`,
  check: (ctx) => {
    const s = ctx.blockStats[b.id];
    return !!s && s.total >= 20 && s.correct / s.total >= 0.85;
  },
}));

const ALL_BADGES = [...FIXED_BADGES, ...BLOCK_BADGES];
...
const QUESTIONS = [
  ...
  },
  {
    id: 5,
    block: "cadera",
    subtopic: "fracturas_femur_proximal",
    code: "Garden",
    image: null,
    prompt:
  ...
  },
  {
    id: 7,
    block: "cadera",
    subtopic: "fracturas_pelvis_acetabulo",
    code: "Letournel",
    image: null,
    prompt:
    ...
    correct: 3,
    explanation:
      "Las fracturas de ambas columnas forman parte del patrón asociado o complejo",
  },
  {
    id: 8,
    block: "cadera",
  ...
  },
  {
    ...
    correct: 2,
    explanation:
      "La columna media incluye la pared posterior del cuerpo vertebral, el anillo fibroso posterior y el ligamento longitudinal posterior; su integridad es clave para la estabilidad vertebral.",
  },
  {
    id: 11,
    block: "raquis",
    ...
    image: null,
    prompt:
      "En la escala de Frankel para lesión medular, un paciente con función motora voluntaria útil preservada pero con alteración sensitiva se clasifica como:",
    options: [
      "Frankel B",
      "Frankel A",
      "Frankel D",
    ...
    correct: 2,
    explanation:
      "Frankel D indica función motora útil preservada con déficit sensitivo variable; Frankel C indica función motora voluntaria no útil funcionalmente; E corresponde a normalidad completa.",
  },
  {
    id: 12,
    block: "raquis",
    ...
    options: [
      "Ausencia completa de función motora y sensitiva por debajo de la lesión, incluyendo los segmentos sacros S4-S5",
      "Ausencia completa de función motora y sensitiva por debajo de la lesión, sin incluir los segmentos sacros S4-S5",
      "Función motora útil en más de la mitad de los músculos clave",
      "Función motora útil en menos de la mitad de los músculos clave",
    ],
    correct: 0,
    explanation:
      "ASIA A define una lesión medular completa en la que no hay preservación motora ni sensitiva por debajo de la lesión ni en los segmentos sacros S4-S5. Si no existe afectación de los segmentos sacros S4-S5, se considera lesión medular incompleta o ASIA B",
  },
  {
    id: 13,
    block: "ms",
    ...
    image: null,
    prompt:
      "Según la clasificación de Neer para fracturas de húmero proximal, ¿qué desplazamiento define un fragmento como fragmento o parte independiente?",
    options: [
      "Cualquier trazo de fractura visible en la radiografía",
      "Más de 2 cm de desplazamiento o más de 90° de angulación",
  ...
  },
  {
    ...
    correct: 1,
    explanation:
      "Mason II corresponde a fracturas con desplazamiento superior a 2 mm sin conminución relevante con o sin bloqueo articular; Mason III implica conminución significativa.",
  },
  {
    id: 16,
    block: "ms",
  ...
  },
  {
    ...
    correct: 2,
    explanation:
      "La fractura-luxación de Monteggia combina una fractura de la diáfisis cubital proximal con luxación de la cabeza del radio.",
  },
  {
    id: 18,
    block: "ms",
    subtopic: "antebrazo_muneca",
    code: "Galeazzi",
    image: null,
    prompt:
    ...
    correct: 2,
    explanation:
      "La fractura-luxación de Galeazzi asocia una fractura diafisaria del radio con una luxación de la articulación radiocubital distal.",
  },
  {
    id: 19,
    block: "ms",
  ...
  },
  {
    ...
    correct: 2,
    explanation:
      "Herbert A2 corresponde a fracturas estables o incompletas de la cintura del escafoides sin desplazamiento significativo.",
  },
  {
    id: 21,
    block: "ms",
  ...
  },
  {
    ...
    options: [
      "Aplasia unilateral de la porción costoesternal del pectoral mayor",
  ...
  },
  {
    ...
    options: [
      "Un elemento del manguito rotador contra el acromion o estructuras periarticulares",
  ...
  },
  {
    ...
    options: [
      ...
      "Si hay retracción hasta la glena, infiltración grasa severa o distancia acromiohumeral menor de 7 mm",
    ],
    correct: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      "External lag test; rotura posterosuperior con afectación de infraespinoso y redondo menor",
  ...
  },
  {
    ...
    options: [
      ...
      "No se han encontrado diferencias clínicas significativas en la reducción del dolor entre ambas técnicas",
  ...
  },
  {
    ...
    options: [
      ...
      "El primero pinzamiento interno; el segundo pinzamiento coracoideo",
    ],
    correct: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      "Continuar tratamiento conservador (fisioterapia, control del dolor)",
  ...
  },
  {
    ...
    options: [
      ...
      "Rotura del manguito de los rotadores",
      "Inestabilidad de la AAC por lesión de los ligamentos coracoclaviculares",
  ...
  },
  {
    ...
    options: [
      ...
      "Ligamento glenohumeral superior avulsionado; rotura crónica del subescapular",
    ],
    correct: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "Iniciar tratamiento conservador con vigilancia estrecha",
  ...
  },
  {
    ...
    options: [
      ...
      "Las luxaciones posteriores pueden asociar compresión mediastínica",
    ],
    correct: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      "Tipo V; tratamiento quirúrgico con refuerzo coracoclavicular tipo suspensión",
  ...
  },
  {
    ...
    options: [
      ...
      "Lesión estable; puede valorarse tratamiento conservador",
  ...
  },
  {
    ...
    options: [
      "Técnica de Mumford (resección del extremo distal de clavícula) aislada",
  ...
  },
  {
    ...
    options: [
      "Son más frecuentes en la clavícula derecha (salvo dextrocardia)",
  ...
  },
  {
    ...
    options: [
      ...
      "Reducción abierta, evitando agujas de Kirschner por riesgo de migración mediastínica",
    ],
    correct: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "En general el tratamiento sigue siendo conservador con resultados similares al quirúrgico",
    ],
    correct: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "Solicitar ecografía o RM para descartar rotura del manguito rotador",
    ],
    correct: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "La lesión es 'off-track': mayor riesgo de recidiva por enganche en el reborde glenoideo",
  ...
  },
  {
    ...
    options: [
      ...
      "Lesión HAGL; desinserción de los ligamentos glenohumerales en su origen humeral, no en la glenoides",
  ...
  },
  {
    ...
    options: [
      "Luxación posterior bilateral; debe evitarse la rotación externa forzada durante la reducción",
  ...
  },
  {
    ...
    options: [
      "Puede plantearse estabilización quirúrgica de entrada, por su perfil de riesgo (edad <25 años, deporte de contacto)",
  ...
  },
  {
    ...
    options: [
      ...
      "Se trata de un defecto 'subcrítico' (13,5-20%) con Hill-Sachs off-track; se prefiere Bankart + Remplissage o Latarjet",
  ...
  },
  {
    ...
    options: [
      ...
      "Triada terrible del hombro; se confirma la afectación nerviosa con electromiograma",
    ],
    correct: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "Lesión de Kim; avulsión oculta e incompleta del labrum posteroinferior",
  ...
  },
  {
    ...
    options: [
      ...
      "Tratamiento no quirúrgico 3-6 meses, con fortalecimiento de estabilizadores dinámicos y control escapular",
  ...
  },
  {
    ...
    options: [
      "Está indicada la estabilización quirúrgica del troquíter desplazado tras la reducción",
  ...
  },
  {
    ...
    options: [
      ...
      "Luxación inferior o 'luxatio erecta'; descartar lesión de arteria axilar y plexo braquial",
  ...
  },
  {
    ...
    options: [
      "En general, tratamiento conservador si la fisis está abierta, salvo sospecha de lesión de Bankart",
  ...
  },
  {
    ...
    options: [
      ...
      "Cirugía urgente con restablecimiento de la circulación del brazo (sutura, injerto o prótesis vascular)",
  ...
  },
  {
    ...
    options: [
      "Está indicado tanto si existe un defecto óseo significativo como si hay múltiples factores de riesgo de recidiva",
  ...
  },
  {
    ...
    options: [
      ...
      "El tratamiento conservador es una opción razonable, sin diferencias significativas frente al quirúrgico",
  ...
  },
  {
    ...
    options: [
      "Densidad mineral ósea baja de la cabeza humeral, con mayor riesgo de fallo de la osteosíntesis",
  ...
  },
  {
    ...
    options: [
      ...
      "En trabajadores manuales el umbral aceptable se reduce a 3 mm, por lo que estaría indicado el tratamiento quirúrgico",
  ...
  },
  {
    ...
    options: [
      ...
      "Está indicado el tratamiento quirúrgico, por pseudoartrosis sintomática de más de 6 meses de evolución",
  ...
  },
  {
    ...
    options: [
      ...
      "'Little League Shoulder' (epifisiolisis por sobreuso); tratamiento con descanso y modificación de actividad",
  ...
  },
  {
    ...
    options: [
      ...
      "Ecografía, porque la epífisis proximal del húmero es cartilaginosa en el neonato",
  ...
  },
  {
    ...
    options: [
      ...
      "Amputación funcional del miembro",
      "Tratamiento quirúrgico, ya que supera el límite de 60º aceptado en menores de 10-11 años",
    ],
    correct: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "Hematoma de Hennequin (o 'en herradura'); hallazgo esperable, no indica complicación grave por sí solo",
    ],
    correct: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      "Se prefiere el tratamiento protésico frente a la osteosíntesis, por el alto riesgo de necrosis avascular",
      "Amputación funcional del miembro",
  ...
  },
  {
    ...
    options: [
      ...
      "Agujas percutáneas, con o sin sutura o injerto óseo asociado",
  ...
  },
  {
    ...
    options: [
      ...
      "Se atribuía el papel principal a la circunfleja anterior, pero estudios recientes muestran que la posterior aporta hasta el 64%",
  ...
  },
  {
    ...
    options: [
      "El tratamiento quirúrgico está indicado, ya que el codo flotante es una indicación reconocida de cirugía",
  ...
  },
  {
    ...
    options: [
      ...
      "Tratar la fractura según los criterios habituales y monitorizar la función radial",
  ...
  },
  {
    ...
    options: [
      ...
      "Solicitar electromiografía a partir de las 3-4 semanas, y valorar cirugía si no mejora en 4-6 meses",
    ],
    correct: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "Debe considerarse indicación de tratamiento quirúrgico",
  ...
  },
  {
    ...
    options: [
      "La congruencia cúbito-humeral y el ligamento colateral cubital lateral",
  ...
  },
  {
    ...
    options: [
      ...
      "Luxación congénita de cabeza radial; actitud expectante, sin tratamiento activo",
  ...
  },
  {
    ...
    options: [
      ...
      "Osteotomía desrotadora con fijación percutánea con agujas",
    ],
    correct: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "Corregir el crecimiento anómalo de la fisis radial distal",
  ...
  },
  {
    ...
    options: [
      ...
      "Hiperplasia angiofibroblástica del origen de los tendones extensores",
    ],
    correct: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "Osteocondritis disecante del capitellum humeral, que debe diferenciarse de la enfermedad de Panner",
    ],
    correct: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "Artrosis primaria de codo; predominio radiocapitelar con relativa preservación cubitohumeral",
    ],
    correct: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "Implante abisagrado (linked/constreñido), menos dependiente de la integridad de los tejidos blandos",
  ...
  },
  {
    ...
    options: [
      ...
      "Puede realizarse sin alterar de forma relevante la estabilidad del codo",
  ...
  },
  {
    ...
    options: [
      "Colocar una férula en flexión y algo de supinación, y repetir la maniobra más tarde",
  ...
  },
  {
    ...
    options: [
      "Reducción cerrada, valorando técnicas percutáneas si fracasa",
  ...
  },
  {
    ...
    options: [
      ...
      "No identificar ni reparar las lesiones ligamentosas asociadas del codo",
    ],
    correct: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "La inclinación cubital fisiológica del radio junto con la musculatura radial (ECRB y ECRL)",
  ...
  },
  {
    ...
    options: [
      ...
      "En la sección de los nervios sensitivos y propioceptivos de la muñeca",
  ...
  },
  {
    ...
    options: [
      "Compresión de la rama sensitiva del nervio radial (Wartenberg/cheiralgia parestésica)",
  ...
  },
  {
    ...
    options: [
      ...
      "Desequilibrio tendinoso con relajación capsular volar de la interfalángica proximal",
  ...
  },
  {
    ...
    options: [
      ...
      "Son la fractura más frecuente en niños; el tipo II de Salter-Harris es el más habitual",
    ],
    correct: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "Yeso largo braquiopalmar inicial, seguido de yeso corto hasta 4-6 semanas",
    ],
    correct: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "Impactación de la estiloides cubital en extensión y desviación cubital de la muñeca",
  ...
  },
  {
    ...
    options: [
      "Férula de Stack en extensión de la interfalángica distal durante al menos 6 semanas",
  ...
  },
  {
    ...
    options: [
      "Fractura de Seymour, por el riesgo de interposición de la matriz ungueal y osteomielitis",
  ...
  },
  {
    id: 285,
    block: "raquis",
    subtopic: "anatomia_exploracion_raquis",
  ...
  },
  {
    id: 290,
    block: "raquis",
    subtopic: "cervical_degenerativa",
  ...
  },
  {
    ...
    options: [
      ...
      "Es positivo si el dolor radicular aparece al elevar la pierna 30-45º con la rodilla en extensión",
  ...
  },
  {
    ...
    options: [
      ...
      "El debilitamiento del psoas ilíaco durante la disección del abordaje",
  ...
  },
  {
    ...
    options: [
      "Subluxación atloaxoidea tipo II (síndrome de Grisel); collarín 7 días",
  ...
  },
  {
    ...
    options: [
      ...
      "A que esta zona es la transición biomecánica entre la caja torácica rígida y la columna lumbar",
    ],
    correct: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "Puede resistir cargas fisiológicas sin lesión neurológica, deformidad inaceptable o dolor crónico",
  ...
  },
  {
    ...
    options: [
      ...
      "En el cono medular se preservan los reflejos bulbo-cavernoso y de la micción; en la cauda equina están abolidos",
    ],
    correct: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      "Un shock neurogénico por simpatectomía funcional, que no responde a cristaloides",
  ...
  },
  {
    ...
    options: [
      ...
      "La cifoplastia crea una cavidad con un dispositivo expansible antes de rellenarla con cemento",
  ...
  },
  {
    ...
    options: [
      ...
      "No, el colapso superior a dos tercios de la altura vertebral es una contraindicación",
    ],
    correct: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "La barra anterior",
      "La barra unilateral",
      "La barra completa o circunferencial",
    ],
    correct: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "No corrige el crecimiento vertebral anómalo, aunque puede usarse en curvas compensadoras flexibles",
  ...
  },
  {
    ...
    options: [
      ...
      "Indicar cirugía de entrada, ya que las barras unilaterales son quirúrgicas de inicio",
    ],
    correct: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      "Nivel lumbar, con morfología trapezoidal o cuneiforme anterior similar a un aplastamiento",
  ...
  },
  {
    ...
    options: [
      ...
      "Una inestabilidad vertebro-pélvica, propia de la forma inestable",
  ...
  },
  {
    ...
    options: [
      "La occipitalización del atlas, con fusión total o parcial al occipucio",
  ...
  },
  {
    ...
    options: [
      ...
      "Un defecto en la unión de la odontoides con el cuerpo del axis, o una pseudoartrosis previa",
  ...
  },
  {
    ...
    options: [
      ...
      "El descenso de las amígdalas cerebelosas junto con la parte inferior del vermis y del IV ventrículo",
  ...
  },
  {
    ...
    options: [
      ...
      "Tratamiento quirúrgico, indicado en curvas torácicas iguales o mayores de 50 grados",
    ],
    correct: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      "La falta de restitución de la lordosis lumbar en fusiones por patología degenerativa",
  ...
  },
  {
    ...
    options: [
      ...
      "Que los hallazgos radiológicos deben correlacionarse con la clínica antes de tomar decisiones terapéuticas",
    ],
    correct: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "La altura de la odontoides respecto a la línea entre paladar duro y occipucio; migración superior si supera 4,5 mm",
  ...
  },
  {
    ...
    options: [
      ...
      "El dolor inflamatorio empeora por la noche con afectación del estado general; el mecánico, con el movimiento o la carga",
    ],
    correct: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "No sentar la indicación quirúrgica solo con el resultado de la RM",
    ],
    correct: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      "La presencia de tres o más signos de Waddell positivos",
  ...
  },
  {
    id: 475,
    block: "infantil",
    subtopic: "coxa_vara_deficiencia_femoral",
    code: "Deficiencia femoral focal proximal tipo D",
    image: null,
    prompt:
      "Recién nacido con una deficiencia femoral focal proximal clasificada como tipo D de Aitken. ¿Qué hallazgo anatómico caracteriza a este tipo?",
    options: [
  ...
  },
  {
    ...
    options: [
      ...
      "Reducción cerrada suave con fijación percutánea, o un procedimiento abierto controlado",
    ],
    correct: 3,
    explanation:
  ...
  },
  {
    id: 511,
    block: "cadera",
    subtopic: "patologia_no_traumatica_cadera",
  ...
  },
  {
    ...
    options: [
      ...
      "Diferencia mejor los estadios y distingue el hundimiento mayor o menor de 2 mm",
    ],
    correct: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      "Una osteopatía dinámica de pubis",
      "Una rotura degenerativa del glúteo medio y menor",
  ...
  },
  {
    ...
    options: [
      "Una irritación del tendón del psoas por extrusión del componente acetabular",
  ...
  },
  {
    ...
    options: [
      ...
      "Realizar una artrocentesis para estudio bioquímico, recuento celular y cultivo",
  ...
  },
  {
    ...
    options: [
      ...
      "Radiografía AP de pelvis, radiografía de tórax y ecofast en la sala de reanimación",
  ...
  },
  {
    ...
    options: [
      ...
      "Realizar un empaquetado pélvico extraperitoneal, previo fijador externo",
  ...
  },
  {
    ...
    options: [
      ...
      "Su mortalidad es similar a la de las fracturas de cadera, por lo que puede estar indicada la cirugía",
  ...
  },
  {
    ...
    options: [
      ...
      "Reposo, hielo y antiinflamatorios, con reincorporación progresiva al deporte",
    ],
    correct: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      "Que alguna parte de la superficie articular quede en continuidad con el esqueleto axial",
  ...
  },
  {
    ...
    options: [
      ...
      "Una impactación superomedial del acetábulo con subluxación posterosuperior de la cabeza femoral",
    ],
    correct: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "Presentan una distribución bimodal, con ancianas osteoporóticas por baja energía y jóvenes por alta energía",
  ...
  },
  {
    ...
    options: [
      ...
      "Aumenta la carga transmitida al implante y el riesgo de rotura y pseudoartrosis",
  ...
  },
  {
    ...
    options: [
      ...
      "El enclavado intramedular anterógrado, fresado y bloqueado",
    ],
    correct: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "Desbridamiento quirúrgico, toma de cultivos y antibioterapia intravenosa",
    ],
    correct: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "Porque el aporte vascular llega por vasos perpendiculares de la arteria femoral profunda en la línea áspera",
  ...
  },
  {
    ...
    options: [
      ...
      "La fractura de cuello femoral ipsilateral",
    ],
    correct: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "Fijación con clavo intramedular anterógrado, fresado y bloqueado, en las primeras 24 horas",
  ...
  },
  {
    ...
    options: [
      ...
      "En una fractura abierta grado III o en politraumatizado grave con control de daños",
  ...
  },
  {
    ...
    options: [
      ...
      "La dismetría de miembros inferiores por hipercrecimiento es la complicación más frecuente",
  ...
  },
  {
    ...
    options: [
      ...
      "Presentan una distribución bimodal, con el pico más frecuente en ancianas por baja energía",
    ],
    correct: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      "Como referencia del punto de entrada de los clavos retrógrados, evitando penetrar la escotadura intercondílea",
  ...
  },
  {
    ...
    options: [
      ...
      "Preserva la vascularización del foco, mejorando la consolidación",
    ],
    correct: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "Traslación medial y rotación externa del segmento distal",
  ...
  },
  {
    ...
    options: [
      ...
      "La mayoría ocurren en adolescentes, y debe descartarse malignidad subyacente",
  ...
  },
  {
    ...
    options: [
      ...
      "En displasias trocleares severas, con espolón supratroclear muy prominente",
    ],
    correct: 3,
    explanation:
...
const STREAK_KEY = "oct_streak_v1";
const FAILED_KEY = "oct_failed_v1";
...
function readStreak() {
  try {
    ...
    if (raw && typeof raw.count === "number" && raw.lastDate) {
      return { count: raw.count, lastDate: raw.lastDate, best: typeof raw.best === "number" ? raw.best : raw.count };
    }
  } catch (e) {}
  return { count: 0, lastDate: null, best: 0 };
}

// Se llama al enviar una respuesta: cuenta el día como "activo" una sola vez,
// y suma a la racha si el día anterior también hubo actividad.
function bumpStreak() {
  const today = todayStr();
  const current = readStreak();
  if (current.lastDate === today) return current; // el día de hoy ya estaba contado
  const nextCount = current.lastDate === yesterdayStr() ? current.count + 1 : 1;
  const next = { count: nextCount, lastDate: today, best: Math.max(current.best || 0, nextCount) };
  try {
...
const PERFECT_KEY = "oct_perfect_session_v1";

function readHadPerfectSession() {
  try {
    return localStorage.getItem(PERFECT_KEY) === "1";
  } catch (e) {
    return false;
  }
}

function markHadPerfectSession() {
  try {
    localStorage.setItem(PERFECT_KEY, "1");
  } catch (e) {}
}

function readFailedIds() {
  try {
    const raw = JSON.parse(localStorage.getItem(FAILED_KEY));
...
function saveFailedIds(ids) {
  try {
    localStorage.setItem(FAILED_KEY, JSON.stringify(ids));
  } catch (e) {}
}

const STATS_KEY = "oct_block_stats_v1";

function readBlockStats() {
  try {
...
function saveBlockStats(stats) {
  try {
    localStorage.setItem(STATS_KEY, JSON.stringify(stats));
  } catch (e) {}
}

const FLAGGED_KEY = "oct_flagged_v1";
...
function saveFlaggedIds(ids) {
  try {
    localStorage.setItem(FLAGGED_KEY, JSON.stringify(ids));
  } catch (e) {}
}

// Objetivo semanal: cuenta preguntas respondidas (cualquier resultado) desde
// el lunes de la semana en curso. El objetivo (target) se guarda aparte y
// persiste indefinidamente; el contador se reinicia solo al cambiar de semana.
const WEEKLY_GOAL_KEY = "oct_weekly_goal_v1";
const DEFAULT_WEEKLY_TARGET = 100;
...
function readWeeklyGoal() {
  ...
  try {
    const raw = JSON.parse(localStorage.getItem(WEEKLY_GOAL_KEY));
    if (raw && typeof raw.count === "number" && typeof raw.target === "number") {
      if (raw.weekStart === thisMonday) return raw;
      // semana nueva: se conserva el objetivo, se reinicia el contador
      return { weekStart: thisMonday, count: 0, target: raw.target };
    }
  } catch (e) {}
  return { weekStart: thisMonday, count: 0, target: DEFAULT_WEEKLY_TARGET };
}
...
function bumpWeeklyGoal() {
  const current = readWeeklyGoal();
...
export default function OpositaCOTTest() {
  ...
  const [failedIds, setFailedIds] = useState(() => readFailedIds());
  const [reviewMode, setReviewMode] = useState(false);
  const [blockStats, setBlockStats] = useState(() => readBlockStats());
  const totalCorrectEver = useMemo(
    () => Object.values(blockStats).reduce((sum, s) => sum + (s.correct || 0), 0),
    [blockStats]
  );
  const levelInfo = useMemo(() => getLevelInfo(totalCorrectEver), [totalCorrectEver]);
  const [hadPerfectSession, setHadPerfectSession] = useState(() => readHadPerfectSession());
  const [showBadges, setShowBadges] = useState(false);

  // --- Autenticación (Fase 1: solo login, no sincroniza datos todavía) ---
  ...
  // --- Sincronización con Supabase (Fase 2) ---
  // "idle": sin cuenta o sin configurar. "syncing": bajando datos justo tras
  // iniciar sesión. "synced": ya sincronizado, cualquier cambio se sube solo.
  const [syncStatus, setSyncStatus] = useState("idle");
  const skipNextPushRef = useRef(false);

  // Al iniciar sesión: si ya hay progreso guardado en la cuenta, manda la nube
  // (sobrescribe este dispositivo). Si es la primera vez con esta cuenta, sube
  // lo que ya había en este dispositivo para no perderlo.
  useEffect(() => {
    if (!supabase || !user) {
      setSyncStatus("idle");
      return;
    ...
    (async () => {
      ...
      if (cancelled) return;
      if (error) {
        console.error("OpositaCOT: fallo al leer el progreso de Supabase:", error);
        setSyncStatus("error");
        return;
      }
      if (data) {
        ...
        if (data.block_stats) {
          setBlockStats(data.block_stats);
          saveBlockStats(data.block_stats);
        }
        if (data.failed_ids) {
          setFailedIds(data.failed_ids);
          saveFailedIds(data.failed_ids);
        }
        if (data.flagged_ids) {
          setFlaggedIds(data.flagged_ids);
          saveFlaggedIds(data.flagged_ids);
        }
        if (data.weekly_goal) {
          setWeeklyGoal(data.weekly_goal);
          saveWeeklyGoal(data.weekly_goal);
        }
        if (data.had_perfect_session) {
          setHadPerfectSession(true);
          markHadPerfectSession();
        }
      } else {
        const { error: upsertError } = await supabase.from("user_progress").upsert({
          ...
          had_perfect_session: hadPerfectSession,
          updated_at: new Date().toISOString(),
        });
        if (upsertError) {
          console.error("OpositaCOT: fallo al subir el progreso inicial a Supabase:", upsertError);
          if (!cancelled) setSyncStatus("error");
          return;
        }
      }
      if (cancelled) return;
      skipNextPushRef.current = true; // evita subir de vuelta lo que acabamos de bajar
      setSyncStatus("synced");
    })();
    return () => {
      cancelled = true;
    };
    // Solo se relanza cuando cambia el usuario (login/logout), no en cada cambio de datos.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [user]);

  // Cualquier cambio posterior en el progreso, ya sincronizado, se sube solo
  // (con un pequeño margen para no lanzar una petición por cada tecla/click).
  useEffect(() => {
    if (!supabase || !user || syncStatus !== "synced") return;
    ...
    const t = setTimeout(() => {
      supabase
        ...
        .then(({ error }) => {
          if (error) {
            console.error("OpositaCOT: fallo al sincronizar el progreso con Supabase:", error);
            setSyncStatus("error");
          }
        });
    }, 800);
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [streak, blockStats, failedIds, flaggedIds, weeklyGoal, hadPerfectSession]);

  const blocksExplored = useMemo(
    () => BLOCKS.filter((b) => blockStats[b.id] && blockStats[b.id].total > 0).length,
    [blockStats]
  );
  const badgeCtx = useMemo(
    () => ({ totalCorrectEver, bestStreak: streak.best || 0, blockStats, blocksExplored, hadPerfectSession }),
    [totalCorrectEver, streak, blockStats, blocksExplored, hadPerfectSession]
  );
  const unlockedBadges = useMemo(
    () => ALL_BADGES.filter((b) => b.check(badgeCtx)),
    [badgeCtx]
  );
  const [flaggedIds, setFlaggedIds] = useState(() => readFlaggedIds());
  const [examMode, setExamMode] = useState(false); // checkbox en la pantalla de inicio
  const [examActive, setExamActive] = useState(false); // examen en curso en esta sesión
  const [examSecondsLeft, setExamSecondsLeft] = useState(0);
  const [selectedSubtopics, setSelectedSubtopics] = useState([]); // ["ms:hombro", ...]
  const [expandedBlockId, setExpandedBlockId] = useState(null);
  const [combo, setCombo] = useState(0);
  const [bestComboSession, setBestComboSession] = useState(0);
  ...
  const toggleBlock = (id) => {
    ...
    // Al quitar un bloque, se quitan también sus subtemas marcados.
    setSelectedSubtopics((prev) => prev.filter((s) => !s.startsWith(id + ":")));
    if (expandedBlockId === id) setExpandedBlockId(null);
  };

  const toggleSubtopic = (blockId, subId) => {
    const key = `${blockId}:${subId}`;
    setSelectedSubtopics((prev) =>
      prev.includes(key) ? prev.filter((s) => s !== key) : [...prev, key]
    );
    // Marcar un subtema selecciona automáticamente el bloque al que pertenece.
    setSelectedBlocks((prev) => (prev.includes(blockId) ? prev : [...prev, blockId]));
  };

  // Preguntas de un bloque que cumplen el filtro de subtemas activo (si lo hay).
  const questionMatchesBlock = (q) => {
    if (!selectedBlocks.includes(q.block)) return false;
    const activeSubs = selectedSubtopics.filter((s) => s.startsWith(q.block + ":"));
    if (activeSubs.length === 0) return true; // sin filtro de subtema: todo el bloque
    return activeSubs.includes(`${q.block}:${q.subtopic}`);
  };

  const startQuiz = () => {
    const filtered = QUESTIONS.filter(questionMatchesBlock);
    ...
    setAnswers([]);
    setCombo(0);
    setBestComboSession(0);
    setReviewMode(false);
    setExamActive(examMode);
    setExamSecondsLeft(examMode ? shuffled.length * 90 : 0);
    setStage("quiz");
  };

  const startReviewQuiz = () => {
    const filtered = QUESTIONS.filter((q) => failedIds.includes(q.id));
  ...
  const startFlaggedQuiz = () => {
    const filtered = QUESTIONS.filter((q) => flaggedIds.includes(q.id));
    if (filtered.length === 0) return;
    setPool(shuffle(filtered));
    setIdx(0);
    setSelected(null);
  ...
  const startQuickQuiz = (n) => {
    const source =
      selectedBlocks.length > 0
        ? QUESTIONS.filter(questionMatchesBlock)
        : QUESTIONS;
    ...
    setReviewMode(false);
    setExamActive(examMode);
    setExamSecondsLeft(examMode ? chosen.length * 90 : 0);
    setStage("quiz");
  };
  ...
  // Registra la respuesta (racha, fallos y estadísticas por bloque). Común a
  // modo normal (submitAnswer) y modo examen (submitExamAnswer).
  const recordAnswer = (chosenIdx) => {
    const isCorrect = chosenIdx === currentQ.correct;
    setAnswers((prev) => [
      ...prev,
      { q: currentQ, chosen: chosenIdx, correct: isCorrect },
    ]);
    setStreak(bumpStreak());
    setWeeklyGoal(bumpWeeklyGoal());
    setCombo((prev) => {
      const next = isCorrect ? prev + 1 : 0;
      setBestComboSession((best) => Math.max(best, next));
      return next;
    });
    setFailedIds((prev) => {
      const has = prev.includes(currentQ.id);
      let next = prev;
      if (isCorrect && has) {
        next = prev.filter((id) => id !== currentQ.id);
      } else if (!isCorrect && !has) {
        next = [...prev, currentQ.id];
      }
      if (next !== prev) saveFailedIds(next);
      return next;
    });
    setBlockStats((prev) => {
      const cur = prev[currentQ.block] || { correct: 0, total: 0 };
      const next = {
        ...prev,
        [currentQ.block]: {
  ...
  // Modo examen: registra la respuesta sin mostrar corrección y avanza directamente.
  ...
  // Cuenta atrás del modo examen: 1 tick por segundo mientras dure el examen.
  useEffect(() => {
    if (stage !== "quiz" || !examActive) return;
    if (examSecondsLeft <= 0) {
      setStage("summary");
      return;
    }
    const t = setTimeout(() => setExamSecondsLeft((s) => Math.max(0, s - 1)), 1000);
    return () => clearTimeout(t);
  }, [stage, examActive, examSecondsLeft]);

  // Atajos de teclado durante el test: 1-4 elige opción, Enter envía / avanza.
  useEffect(() => {
    if (stage !== "quiz" || !currentQ) return;
    const handleKeyDown = (e) => {
      if (e.key >= "1" && e.key <= "4") {
        const i = Number(e.key) - 1;
        if (i < currentQ.options.length) choose(i);
      } else if (e.key === "Enter") {
        e.preventDefault();
        if (selected === null) {
          if (examActive) submitExamAnswer();
          else submitAnswer();
        } else {
          next();
        }
  ...
  }, [stage, currentQ, picked, selected, examActive]);
  ...
  // Comprueba la insignia de sesión perfecta al llegar al resumen.
  useEffect(() => {
    if (stage === "summary" && stats.total >= 10 && stats.totalCorrect === stats.total && !hadPerfectSession) {
      markHadPerfectSession();
      setHadPerfectSession(true);
    }
  }, [stage, stats, hadPerfectSession]);
    ...
    >
      ...
      >
        ...
        >
          ...
          <div>
            <div
              ...
              style={{ fontSize: "18px", fontWeight: 700, letterSpacing: "-0.01em", lineHeight: 1.1, color: "#0F172A" }}
            >
              OpositaCOT
            </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "14px", flexWrap: "wrap" }}>
          <div
            title={`${totalCorrectEver} aciertos acumulados`}
            style={{
              display: "inline-flex",
              alignItems: "center",
              gap: "6px",
          ...
          {supabase && (
            <button
              ...
              style={{
                ...
                padding: "6px 14px",
                borderRadius: "20px",
                background: user ? "#ECFDF5" : "#F1F5F9",
                color: user ? "#047857" : "#475569",
                whiteSpace: "nowrap",
                border: "none",
                cursor: "pointer",
                fontFamily: "'Inter', sans-serif",
          ...
          {stage === "quiz" && (
            <div
              style={{
                ...
                color: "#475569",
                textAlign: "right",
                background: examActive ? "#FFF7ED" : "#F1F5F9",
                borderRadius: "20px",
                padding: "6px 14px",
              }}
            >
              ...
              {examActive ? (
                <>
                  <span style={{ margin: "0 8px", color: "#FDBA74" }}>·</span>
                  <span style={{ color: "#C2410C", fontWeight: 700 }}>
                    🕐 {formatExamTime(examSecondsLeft)}
                  </span>
                </>
              ) : (
                <>
              ...
              {reviewMode && (
                <>
                  <span style={{ margin: "0 8px", color: "#CBD5E1" }}>·</span>
                  <span style={{ color: "#B91C1C", fontWeight: 600 }}>🔁 Repaso</span>
                </>
              )}
              {combo >= 3 && (
                <>
                  <span style={{ margin: "0 8px", color: "#CBD5E1" }}>·</span>
                  <span style={{ color: "#EA580C", fontWeight: 700 }}>🔥 Combo x{combo}</span>
                </>
              )}
            </div>
          )}
      ...
      <div style={{ padding: "32px 24px 60px", maxWidth: "720px", margin: "0 auto" }}>
        {stage === "setup" && (
          <div>
            ...
            <p style={{ color: "#64748B", fontSize: "14.5px", lineHeight: 1.6, marginBottom: "20px" }}>
              Elige uno o varios bloques del temario para elaborar tu sesión de test.
            ...
            >
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "8px" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                  ...
                  <span className="oct-display" style={{ fontSize: "15px", fontWeight: 600, color: "#0F172A" }}>
                    {levelInfo.current.name}
                  </span>
                </div>
                <span style={{ fontSize: "12px", color: "#94A3B8" }}>
                  {totalCorrectEver} acierto{totalCorrectEver === 1 ? "" : "s"} en total
                </span>
              </div>
              <div
                style={{
              ...
              <div style={{ fontSize: "11.5px", color: "#94A3B8", marginTop: "6px" }}>
                {levelInfo.next
                  ? `Te faltan ${levelInfo.next.min - totalCorrectEver} aciertos para "${levelInfo.next.name}"`
            ...
            >
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "8px" }}>
                ...
                {editingWeeklyTarget ? (
                  <input
                    ...
                    autoFocus
                    onBlur={(e) => {
                      const val = Math.max(10, parseInt(e.target.value, 10) || DEFAULT_WEEKLY_TARGET);
                      const next = { ...weeklyGoal, target: val };
                      saveWeeklyGoal(next);
                      setWeeklyGoal(next);
                  ...
                  <button
                    onClick={() => setEditingWeeklyTarget(true)}
                    style={{
                      background: "none",
                      border: "none",
                      padding: 0,
                    ...
                    title="Cambiar objetivo semanal"
                  >
                    {weeklyGoal.count}/{weeklyGoal.target} preguntas ✎
                  </button>
                )}
              </div>
              <div
              ...
              >
                <div
                  style={{
                    width: `${Math.min(100, Math.round((weeklyGoal.count / weeklyGoal.target) * 100))}%`,
                    height: "100%",
                    background: "linear-gradient(90deg, #0D9488, #0284C7)",
                    borderRadius: "20px",
                  }}
              ...
              <div style={{ fontSize: "11.5px", color: "#94A3B8", marginTop: "6px" }}>
                {weeklyGoal.count >= weeklyGoal.target
                  ? "¡Objetivo semanal conseguido! 🎉"
                  : `Quedan ${weeklyGoal.target - weeklyGoal.count} preguntas para tu objetivo de esta semana`}
              </div>
            </div>

            {failedIds.length > 0 && (
              <button
                className="oct-btn oct-card"
              ...
              >
                <div>
                  <div className="oct-display" style={{ fontSize: "15px", fontWeight: 600, color: "#991B1B" }}>
                    🔁 Repasar mis fallos
                  </div>
                  <div style={{ fontSize: "12.5px", color: "#B91C1C", marginTop: "2px" }}>
                    {failedIds.length} pregunta{failedIds.length === 1 ? "" : "s"} pendiente
                    {failedIds.length === 1 ? "" : "s"} de dominar
                  </div>
                </div>
                <span style={{ fontSize: "20px", color: "#B91C1C", flexShrink: 0 }}>→</span>
              </button>
            )}
            ...
            {flaggedIds.length > 0 && (
              ...
              >
                <div>
                  <div className="oct-display" style={{ fontSize: "15px", fontWeight: 600, color: "#92400E" }}>
                    ⭐ Repasar mis dudosas
                  </div>
                  <div style={{ fontSize: "12.5px", color: "#B45309", marginTop: "2px" }}>
                    {flaggedIds.length} pregunta{flaggedIds.length === 1 ? "" : "s"} marcada
                    {flaggedIds.length === 1 ? "" : "s"} para repasar
                  </div>
                </div>
                <span style={{ fontSize: "20px", color: "#B45309", flexShrink: 0 }}>→</span>
              </button>
            )}
            ...
            >
              {BLOCKS.map((b) => {
                ...
                const stat = blockStats[b.id];
                const statPct =
                  stat && stat.total > 0 ? Math.round((stat.correct / stat.total) * 100) : null;
                const subs = SUBTOPICS[b.id];
                const isExpanded = expandedBlockId === b.id;
                const activeSubCount = subs
                  ? selectedSubtopics.filter((s) => s.startsWith(b.id + ":")).length
                  : 0;
                return (
                  <Fragment key={b.id}>
                    <div
                      ...
                      <div>
                        ...
                        {statPct !== null && (
                          <div style={{ marginTop: "8px" }}>
                            ...
                            >
                              <span>Tu histórico</span>
                              <span style={{ fontWeight: 600, color: b.color }}>{statPct}%</span>
                            </div>
                            <div
                              style={{
                                height: "5px",
                        ...
                        {subs && (
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              setExpandedBlockId(isExpanded ? null : b.id);
                            }}
                            style={{
                              marginTop: "10px",
                              background: "none",
                          ...
                          >
                            {activeSubCount > 0
                              ? `${activeSubCount} subtema${activeSubCount === 1 ? "" : "s"} elegido${activeSubCount === 1 ? "" : "s"}`
                              : "Elegir subtemas"}
                    ...
                    {subs && isExpanded && (
                      ...
                      >
                        <div style={{ fontSize: "12.5px", color: "#475569", marginBottom: "10px" }}>
                          Marca uno o varios subtemas de {b.label.toLowerCase()} — si no marcas ninguno, se usa el bloque completo.
                        </div>
                        <div style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>
                          {subs.map((s) => {
                            const key = `${b.id}:${s.id}`;
                            const subActive = selectedSubtopics.includes(key);
                            const subCount = QUESTIONS.filter(
                              (q) => q.block === b.id && q.subtopic === s.id
                            ).length;
                            return (
                              <button
                                key={s.id}
                                onClick={(e) => {
                                  e.stopPropagation();
                                  toggleSubtopic(b.id, s.id);
                                }}
                                style={{
                                  padding: "8px 12px",
                                  borderRadius: "20px",
                                  border: subActive ? `1.5px solid ${b.color}` : "1.5px solid #E2E8F0",
                                  background: subActive ? b.color : "#FFFFFF",
                                  color: subActive ? "#FFFFFF" : "#334155",
                                  fontSize: "12.5px",
                                  fontWeight: 600,
                                  cursor: "pointer",
                                  whiteSpace: "nowrap",
            ...
            >
              ...
              />
              🕐 Modo examen — sin corrección hasta el final, con tiempo (1,5 min/pregunta)
            </label>
            ...
            >
              {selectedBlocks.length === 0
                ...
                : `Comenzar test (${QUESTIONS.filter(questionMatchesBlock).length} preguntas)`}
            </button>
            ...
            >
              ...
              <span style={{ fontSize: "12px", color: "#94A3B8" }}>o ve al grano</span>
              <div style={{ flex: 1, height: "1px", background: "#E2E8F0" }} />
            </div>
              <div style={{ display: "flex", gap: "12px" }}>
                <button
                  className="oct-card"
                  onClick={() => startQuickQuiz(10)}
                  style={{
                    flex: 1,
                    display: "flex",
                    alignItems: "center",
                ...
                >
                  ...
                  <div>
                    <div className="oct-display" style={{ fontSize: "14.5px", fontWeight: 600, color: "#1E293B" }}>
                      10 preguntas
                    </div>
              ...
              <p style={{ fontSize: "12px", color: "#94A3B8", marginTop: "10px", lineHeight: 1.5 }}>
                Preguntas al azar de los bloques marcados arriba, o de todo el banco si no marcas ninguno.
              </p>
          </div>
        )}
        ...
        {stage === "quiz" && currentQ && (
          <div>
            ...
            >
              <div
                style={{ fontSize: "18px", fontWeight: 600, lineHeight: 1.5, color: "#0F172A", flex: 1 }}
              >
                {currentQ.prompt}
              </div>
              <button
                ...
                title={
                  flaggedIds.includes(currentQ.id)
                    ? "Quitar de dudosas"
                    : "Marcar como dudosa para repasar"
                }
                style={{
                  flexShrink: 0,
                  width: "36px",
                  height: "36px",
                  borderRadius: "10px",
                  border: flaggedIds.includes(currentQ.id) ? "1.5px solid #FCD34D" : "1.5px solid #E2E8F0",
                  background: flaggedIds.includes(currentQ.id) ? "#FFFBEB" : "#FFFFFF",
                  color: flaggedIds.includes(currentQ.id) ? "#D97706" : "#CBD5E1",
                  fontSize: "17px",
                  cursor: "pointer",
                  display: "flex",
                  alignItems: "center",
            ...
            {selected === null && (
              <button
                className="oct-btn"
                onClick={examActive ? submitExamAnswer : submitAnswer}
            ...
            <div style={{ textAlign: "center", fontSize: "11.5px", color: "#CBD5E1", marginTop: "10px" }}>
              Atajos: 1-4 para elegir · Enter para enviar o continuar
            </div>
        ...
        {stage === "summary" && (
          <div>
            ...
            >
              ...
              {bestComboSession >= 3 && (
                <div style={{ color: "#EA580C", fontSize: "13px", fontWeight: 600, marginTop: "8px" }}>
                  🔥 Mejor combo de la sesión: {bestComboSession} seguidas
                </div>
              )}
            </div>
            ...
            {failedIds.length > 0 && (
              ...
              >
                🔁 Repasar mis fallos ({failedIds.length})
              </button>
            )}
            ...
            {flaggedIds.length > 0 && (
              ...
              >
                ⭐ Repasar mis dudosas ({flaggedIds.length})
              </button>
            )}
          </div>
        )}
        ...
        >
          ...
          >
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "6px" }}>
              <h2 className="oct-display" style={{ fontSize: "19px", fontWeight: 700, color: "#0F172A", margin: 0 }}>
                🏅 Mis insignias
              </h2>
              <button
                onClick={() => setShowBadges(false)}
            ...
            <p style={{ fontSize: "13px", color: "#94A3B8", marginBottom: "18px" }}>
              {unlockedBadges.length} de {ALL_BADGES.length} conseguidas
            </p>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(230px, 1fr))", gap: "10px" }}>
              {ALL_BADGES.map((b) => {
                ...
                return (
                  ...
                  >
                    <div style={{ fontSize: "22px", flexShrink: 0 }}>{unlocked ? b.icon : "🔒"}</div>
                    <div>
                      <div className="oct-display" style={{ fontSize: "13.5px", fontWeight: 600, color: "#1E293B" }}>
                        {b.name}
                      </div>
      ...
      {showAuthModal && (
        ...
        >
          ...
          >
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "6px" }}>
              <h2 className="oct-display" style={{ fontSize: "18px", fontWeight: 700, color: "#0F172A", margin: 0 }}>
                👤 Mi cuenta
              </h2>
              <button
                onClick={() => setShowAuthModal(false)}
            ...
            {user ? (
              <>
                <p style={{ fontSize: "13.5px", color: "#475569", marginTop: "14px", lineHeight: 1.6 }}>
                ...
                <p
                  style={{
                    ...
                    lineHeight: 1.5,
                    color:
                      syncStatus === "error" ? "#DC2626" : syncStatus === "synced" ? "#0D9488" : "#94A3B8",
                  }}
                >
                  {syncStatus === "syncing" && "☁️ Sincronizando tu progreso..."}
                  {syncStatus === "synced" &&
                    "☁️ Tu progreso (racha, nivel, fallos, dudosas, objetivo semanal) está sincronizado con esta cuenta."}
                  {syncStatus === "error" &&
                    "⚠️ No se ha podido sincronizar. Tus datos siguen a salvo en este dispositivo."}
                </p>
                <button
                  onClick={signOut}
                  style={{
              ...
              <>
                <p style={{ fontSize: "13.5px", color: "#475569", marginTop: "14px", marginBottom: "14px", lineHeight: 1.6 }}>
                  Te enviamos un enlace a tu correo para entrar, sin contraseña.
                </p>
                <input
                  type="email"
                  ...
                  onChange={(e) => {
                    ...
                    if (authStatus !== "idle" && authStatus !== "sending") setAuthStatus("idle");
                  }}
                  onKeyDown={(e) => {
                ...
                <button
                  ...
                  style={{
                    ...
                    border: "none",
                    background:
                      !authEmail || authStatus === "sending"
                        ? "#E2E8F0"
                        : "linear-gradient(135deg, #0D9488, #0284C7)",
                    color: !authEmail || authStatus === "sending" ? "#94A3B8" : "#FFFFFF",
                    fontFamily: "'Poppins', sans-serif",
                    fontWeight: 600,
                    fontSize: "13.5px",
                    cursor: !authEmail || authStatus === "sending" ? "not-allowed" : "pointer",
                  }}
                >
                  {authStatus === "sending" ? "Enviando..." : "Enviarme el enlace"}
                </button>
                {authStatus === "sent" && (
                  <p style={{ fontSize: "12.5px", color: "#0D9488", marginTop: "10px" }}>
                    Revisa tu correo y pulsa el enlace para entrar.
                  </p>
                )}
                {authStatus === "error" && (
                  <p style={{ fontSize: "12.5px", color: "#DC2626", marginTop: "10px" }}>
